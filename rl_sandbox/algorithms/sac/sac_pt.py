...
class SACPT(SACPER):
    def __init__(self, model, policy_opt, qs_opt, alpha_opt, learn_alpha, buffer, algo_params, aux_tasks=AuxiliaryTask()):
        ...
        self._priority_decay = algo_params.get(c.PRIORITY_DECAY, c.DEFAULT_PT_PARAMS[c.PRIORITY_DECAY])
        self._min_priority = algo_params.get(c.MIN_PRIORITY, c.DEFAULT_PT_PARAMS[c.MIN_PRIORITY])

    def _compute_qs_loss(self, obss, h_states, acts, rews, dones, next_obss, discounting, lengths, tree_idxes, is_weights, priorities):
        ...
        with torch.no_grad():
            ...
            new_priorities = (torch.clamp(priorities ** self._priority_decay, min=self._min_priority)).cpu().numpy()
    ...
    def update_qs(self, batch_start_idx, obss, h_states, acts, rews, dones, next_obss, next_h_states, discounting, infos, lengths, update_info):
        ...
        priorities = infos[c.PRIORITY].unsqueeze(-1)
