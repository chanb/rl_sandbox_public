...
class CEMQ:
    ...
    def compute_action(self, q_function, obss, h_states, act_mean, act_var, lengths):
        ...
        if lengths is not None:
            lengths = lengths.repeat(*([1] * (len(lengths.shape) - 1)), self._pop_size).reshape(self._batch_size * self._pop_size, *lengths.shape[1:])

        cov_noise = self.cov_noise_init
        with torch.no_grad():
            for iter_i in range(self._num_iters):
                eps = torch.randn(self._pop_size, self._batch_size, self._action_dim, device=self.device)
                acts = act_mean[None, ...] + eps * act_var[None, ...]
                acts = torch.clamp(acts, min=self.min_action, max=self.max_action)
                values = q_function(obss, h_states, acts.reshape(self._batch_size * self._pop_size, self._action_dim), lengths).reshape(self._pop_size, self._batch_size)

                _, top_idxes = torch.sort(values, dim=0)
                top_idxes = top_idxes[-self._elite_size:]
                top_acts = torch.gather(acts, 0, top_idxes.unsqueeze(-1).expand(self._elite_size, self._batch_size, self._action_dim))

                # CMA-ES uses old mean to compute covariance
                shifted_acts = top_acts - act_mean[None, ...]
                act_var = torch.sum(self.weights * (shifted_acts ** 2), dim=0) / self._elite_size + cov_noise
                cov_noise = self.cov_noise_tau * cov_noise + (1 - self.cov_noise_tau) * self.cov_noise_end

                act_mean = torch.mean(top_acts, dim=0)
                act_mean = torch.sum(self.weights * top_acts, dim=0)
